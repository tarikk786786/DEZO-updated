...
export const GoogleRankingChecklist = () => {
  ...
  const generate = async (e: any) => {
    e.preventDefault();
    setLoading(true);
    const data = await generateWithAI('GoogleRankingChecklist', formData);
    setResult(data);
    ...
    <div className="flex flex-col md:flex-row gap-6">
      <div className="w-full md:w-1/2 overflow-y-auto max-h-[80vh] px-1 pb-4">
        <form onSubmit={generate} className="space-y-4">
          ...
          >
            {loading ? <span className="animate-pulse">Building AI Checklist...</span> : <><Bot size={18} /> Generate SEO Plan</>}
          </button>
        </form>
      </div>
        ...
        {result ? (
          <div className="flex flex-col h-full">
            <div className="flex justify-between items-center mb-4">
              <h4 className="font-black text-white">AI SEO Action Plan</h4>
